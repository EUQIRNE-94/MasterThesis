...
%
% Seguimiento de particulas a formación simétrica -> Sub-Formación en linea ->
% Sub-Formación circulo 
%
...
% Comunicacion limitada en uniciclo
...
d2 = 0.35;
K = 5;
Ks = [5,0.1,0.5,1,0.1,0.5];  % [Cx,Cxk,Cxi,Cy,Cyk,K]
...
Acom1 = anillo(N);
Acom2 = anilloB(N);
%%% Angulo de separación entre uniciclos %%%
ang_f = 360/(2*i); %%%%%%% Importante el valor que puede tomar --> depende de la cantidad de uniciclos
...
for k = 1:n-1
  ...
  if tspan(k) < tcambio(1)
    ...
    if tspan(k) < tcambio(2)
      [Xr(k,:),Yr(k,:),XIr(k,:)] = SubLinea(V0,w0,theta(k,:),R0,2,ang_f,i);
    else
      [Xr(k,:),Yr(k,:),XIr(k,:)] = SubCircleForm(r(k,:),theta(k,:),R0,2,0.65,i);
    end
  end
  ...
  %%% Uniciclos
  [v(k,:),w(k,:)] = TrackingControlV2(V0,etheta(theta(k,:),V0),u(k,:),err_xi(k,:),err_x(k,:),err_y(k,:),Ks,Acom1);
...
xlabel('Time [t]','fontsize',22)
ylabel('Factor','fontsize',22)
legend({'ev_1','ev_2','ev_3','ev_4','ev_5','ev_6'},'Orientation','Vertical' ...
...
xlabel('Time [t]','fontsize',22)
ylabel('v [m/sec]','fontsize',22)
legend({'v_1','v_2','v_3','v_4','v_5','v_6'},'Orientation','Horizontal' ...
  ,'location','south','fontsize',14)

% Guardar imagen
txt = "img/Final08_control" + ".png";
saveas(gcf,txt);
