...
%
% Formaciones a realizar en l subgrupos:
%       - Linea
%       - Punta
%       - Reset
%
  ...
  if tspan(k) > tcambio(1)
    if tspan(k) > tcambio(2)
      if tspan(k) > tretorno
        ...
        [Xr(k,:),Yr(k,:),XIr(k,:)] = SubPointForm(r(k,:),theta(k,:),R0,2,150,0.5,i);
      end
