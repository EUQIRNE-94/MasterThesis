...
A = ones(N);                                                                                        % Matriz de adyacendia de comunicación
A1 = matrFor([3,5],A);                                                                              % Matriz de adyacendia de formación
...
for k = 1:n-1
  ...
  if tspan(k) < 50
    ...
    u(k,:) = SteeringControlV2(w0,r(k,:),theta(k,:),R0,kappa,N,M,A);                                 % Control de direccion
  end
...
for k = 1:n
  ...
  viscircles([x(k,:)',y(k,:)'],0.25*ones(N,1),'color','k');
