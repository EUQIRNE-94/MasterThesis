...
% Evasión de colisiones planteado por Freitas
...
for k = 1:n-1
  ...
  rep(k,:) = evasion(r(k,:),theta(k,:),0.5,0.8);
...
%   viscircles([real(r(k,:))',imag(r(k,:))'],0.25*ones(N,1));
%   text(3,3,num2str(round(tspan(k),1)),'Color','black')
%   text(4,3,"seg")
%   pause(0.05)
% end
% 
...
function rep = evasion(r,theta,d,K)
  ...
  for k = 1:N
    for j = 1:N
      ...
      if M(k,j) < d
        count = count + 1;
        rep(k) = rep(k) + 1.5*sin(theta(k) - theta(j));
      end
    end
    rep(k) = (K/count)*rep(k);
    count = 0;
