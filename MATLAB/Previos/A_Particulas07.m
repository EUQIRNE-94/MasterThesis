...
for k = 1:n-1
  ...
  u1(k,:) = con_lim(w0,theta1(k,:),c1(k,:),kappa,A3);
...
function res = con_lim(w0,theta,c,kappa,A)
  L = laplaciano(A);
  d = sum(A,2);
...
%   res = w0 - ((0.25*kappa./d).*dp)'; % Anillo
%   res = w0 - ((kappa./d).*dp)' + u1'; % Anillo Bidireccional
  res = w0 - ((kappa./d).*dp)'; % Anillo Bidireccional
