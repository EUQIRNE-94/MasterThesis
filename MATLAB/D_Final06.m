...
% Ingreso y egreso de elementos
% Comunicación limitada en uniciclos
...
%%% Matriz de adyacencia de comunicación %%%
A = ones(N);
%%% Matriz de adyacencia de formacion %%%
vec = [3,5];
A1 = matrFor(vec,A);
...
for k = 1:n-1
  ...
  if tspan(k) < tcambio(1)
    u(k,:) = SteeringControlV2(V0,w0,r(k,:),theta(k,:),R0,kappa,N,M-length(vec),A1);
    u(k,vec(1)) = 1;
    u(k,vec(2)) = 1;
  else
  ...
  if tspan(k) < tcambio(2)
    ...
    if tspan(k) < tcambio(3)
      [Xr(k,:),Yr(k,:),XIr(k,:)] = LineForm(V0,w0,r(k,:),theta(k,:),R0,1,ang_f,i);
    else
      [Xr(k,:),Yr(k,:),XIr(k,:)] = CircleForm(r(k,:),theta(k,:),R0,1,0.65,i);
      d2 = 0.2;
    end
  end
