...
for k = 1:n-1
  %%% Partículas en el plano %%%
  u(k,:) = SteeringControl(V0,w0,r(k,:),theta(k,:),R0,kappa,N,M,6);
  ...
  %%% Uniciclos
  [v(k,:),w(k,:)] = TrackingControl(V0,etheta(theta(k,:),V0),u(k,:),err_xi(k,:),err_x(k,:),err_y(k,:),Ks);
...
% end
plot(r(n,:),'go','linewidth',5)

xmin = real(r(n-1,:));
ymin = imag(r(n-1,:));
xmax = real(r(n,:));
ymax = imag(r(n,:));
quiver(xmin,ymin,(xmax-xmin),(ymax-ymin),'g','linewidth',2)
for i = 1:N
  text(Xr(n,i)+0.2,Yr(n,i),num2str(i),'Color','red','Fontsize',20,'Fontweight','bold')
end

% Guardar imagen
% txt = "img/Final00_planoFase" + ".png";
% saveas(gcf,txt);
