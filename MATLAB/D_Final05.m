...
for k = 1:n-1
  ...
  if tspan(k) < tcambio(1)
    ...
    if tspan(k) < tcambio(2)
      ...
      [Xr(k,:),Yr(k,:),XIr(k,:)] = PointForm(r(k,:),theta(k,:),R0,1,150,0.75,i);
    end
  end
