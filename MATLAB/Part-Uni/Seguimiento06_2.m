...
% N partículas con i = N Uniciclos en formación de linea
...
for k = 1:n-1
  ...
  if cambio == 0
    ...
    if k > ncambio(1) && k <= ncambio(2)
      ...
      [Xr(k,:),Yr(k,:),thetar(k,:)] = linea(w0,theta(k,:),1,kf,i);
    end
  end
  ...
  if k == n-1
    ...
    [Xr(k+1,:),Yr(k+1,:),thetar(k+1,:)] = linea(w0,theta(k+1,:),1,kf,i);
...
% 
% % Guardar imagen
% txt0 = "img/seg_06_0" + num2str(N) + ".png";
% saveas(gcf,txt0);
...
% 
% % Guardar imagen
% txt1 = "img/err_06_0" + num2str(N) + ".png";
% saveas(gcf,txt1);
...
grid on
jj = 1;
for k = ncambio(1)-50:1:length(tspan)
  cla(gca(figura));
  for j = 1:i
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Obtener referencias de posicion y fase
for o = 1:i
  deltatheta(1,o) = theta(part) - o*deltagamma;
