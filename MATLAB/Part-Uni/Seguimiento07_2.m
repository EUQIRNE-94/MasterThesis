...
% plot(r)
% for j = 1:N
%   uni(x(n,j),y(n,j),eta(n,j),0.1,'blue');
...
kk = 0;
jj = 1;
for k = ncambio(2)-100:5:length(tspan)
  if tspan(k) > 35 && kk == 0
    axis([-3 3 -3 3]) 
    kk = 1;
  end
  ...
  for j = 1:i
    ...
    text(x(k,j)+.1,y(k,j)+.1,num2str(j))
    text(Xr(k,j)+.2,Yr(k,j)+.2,num2str(j))
  end
