...
% N partículas con i = N Uniciclos en formación linea -> punta
...
for k = 1:n-1
  ...
  if cambio == 0
    ...
    if k > ncambio(1) && k <= ncambio(2)
      ...
      [Xr(k,:),Yr(k,:),thetar(k,:)] = punta(r(k,1),theta(k,:),R0,1,150,0.25,i);
    end
  end
  ...
  if k == n-1
    ...
    [Xr(k+1,:),Yr(k+1,:),thetar(k+1,:)] = punta(r(k+1,1),theta(k+1,:),R0,1,150,0.25,i);
...
function [Xr,Yr,thetar] = punta(r,theta,R,part,angulo,radio,i)
...
for o = 1:i
  deltaX(1,o) = real(r) + a(o)*cos(theta(part) + ((-1)^(-deltas(o)))*beta );
  deltaY(1,o) = imag(r) + a(o)*sin(theta(part) + ((-1)^(-deltas(o)))*beta );
