...
def main():
    ...
    Ks = np.array([[0.1,0,0.025,1,0,1]])  # [Cx,Cxk,Cxi,Cy,Cyk,K] Ganancias de control del uniciclo
    ...
    while tact <= tf:
        ...
        if (tact_a - tant_a) >= dt/5:
            ...
            try:
                ...
                x_par = np.round(x_par,2)
                y_par = np.round(y_par,2)
                xi_par = np.round(xi_par,2)
    ...
    np.savez('DATA/PE2_data',robots=N,tiempo=t,tiempo2=t2,Xref=Xr,Yref=Yr,Xiref=Xir,xest=x,yest=y,xiest=xi,errx=err_x,erry=err_y,errxi=err_xi,vellin=v,velang=w) #,veleva=v_eva
