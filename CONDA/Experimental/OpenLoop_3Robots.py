...
### Variables de robots
r_ll =np.array([0.067,0.067,0.065])             # Llanta del robot 67 mm para RAM01 y 02 65mm el resto
    ...
    while True:
        ...
        if (tact>= 4) and cambio == 0:
            for i in range(vd.size):
                cadd = "@W" + str(np.round(vd[i],4)) + "D" + str(np.round(vd[i],4)) + "I#"
                cad = np.append(cad,cadd)
            RAM01.write(cad[0].encode('ascii'))
            RAM02.write(cad[1].encode('ascii'))
            RAM03.write(cad[2].encode('ascii'))
            cad = []
            cambio = 1
            print('state done 01')
        ...
        if (tact>= 5) and cambio == 1:
            for i in range(vd.size):
                cadd = "@W" + str(np.round(vd[i],4)) + "D" + str(np.round(vi[i],4)) + "I#"
                cad = np.append(cad,cadd)
            RAM01.write(cad[0].encode('ascii'))
            RAM02.write(cad[1].encode('ascii'))
            RAM03.write(cad[2].encode('ascii'))
