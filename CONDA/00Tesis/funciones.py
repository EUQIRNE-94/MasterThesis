...
def trackingControl(V0,etheta,dtheta,err_xi,err_x,err_y,Ks,A):
    ...
    w = dtheta + Ctheta*err_xi + ((norm_v*Cy)*(Kalpha)*(np.sin(err_xi)/(err_xi+.0000001)))*(err_y + Cyk*copY)
    return [v,w]
...
def circForm(r,theta,R,part,radio,i):
    ...
    for k in range(i):
        ...
        d_XI[k] = ang[k] - angulos[k]
        
        if np.rad2deg(d_XI[k]) < 0:
            d_XI[k] = d_XI[k] + 2*pi
