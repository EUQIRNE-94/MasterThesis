...
for k in range(n-1):
    ...
    if tspan[k] < 50:
        ...
        if tspan[k] < 75:
            [Xr[k,:],Yr[k,:],XIr[k,:]] = fx.subLineaForm(V0,w0,theta[k,:],R0,2,ang_f,i)
        else:
            [Xr[k,:],Yr[k,:],XIr[k,:]] = fx.subCircForm(r[k,:],theta[k,:],R0,2,0.65,i)
            d2 = 0.25
...
plt.plot(x,y,'g--',zorder=2)
plt.scatter(Xr[n-1,:],Yr[n-1,:],marker='o',linewidth=1 ,color='m',zorder=3)
...
for k in range(i):
    ...
    plt.text(r[n-1,k].real,r[n-1,k].imag+0.25,k+1,color='r')
    plt.text(x[n-1,k]+0.1,y[n-1,k],k+1)
plt.title('Phase Plane')
plt.xlabel(r'$x [m]$')
plt.ylabel(r'$y [m]$')
plt.grid()
