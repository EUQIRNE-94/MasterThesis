"""
Created on Mon Feb  1 18:58:23

@author: ENRIUE BENAVIDES TELLEZ
"""
...
rep = np.zeros((n,N)); v_rep = np.zeros((n,N))
...
for k in range(n-1):
    ...
    #### Uniciclos ####
    [v[k,:],w[k,:]] = fx.trackingControl(V0,fx.etheta(theta[k,:],V0),u[k,:],err_xi[k,:],err_x[k,:],err_y[k,:],Ks,A)
    [x[k+1,:],y[k+1,:],xi[k+1,:]] = fx.unicycleModel(x[k,:],y[k,:],xi[k,:],v[k,:],w[k,:],dt)
...
x = np.delete(x,n-1,0); y = np.delete(y,n-1,0); xi = np.delete(xi,n-1,0); rep = np.delete(rep,n-1,0);
err_x = np.delete(err_x,n-1,0); err_y = np.delete(err_y,n-1,0); err_xi = np.delete(err_xi,n-1,0);
v = np.delete(v,n-1,0); v_rep = np.delete(v_rep,n-1,0); w = np.delete(w,n-1,0);
