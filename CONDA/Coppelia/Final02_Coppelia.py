...
Formación simétrica -> linea -> circular
"""
...
for k in range(n-1):
    ...
    if estado == 0:
        ...
        ang_par = 45 - (tspan[k] - 30)*(ang_f/30)
        ...
        [Xr_par,Yr_par,Xir_par] = fx.circForm(r_act,theta_act,R0,0,0.75,N)
...
if perturbacion == True:
    np.savez('DATA/02_dataPert',robots=N,tiempo=tspan,Xref=Xr,Yref=Yr,Xiref=Xir,xest=x,yest=y,xiest=xi,errx=err_x,erry=err_y,errxi=err_xi,vellin=v,velang=w,veleva=v_eva)
else:
    np.savez('DATA/02_data',robots=N,tiempo=tspan,Xref=Xr,Yref=Yr,Xiref=Xir,xest=x,yest=y,xiest=xi,errx=err_x,erry=err_y,errxi=err_xi,vellin=v,velang=w,veleva=v_eva)