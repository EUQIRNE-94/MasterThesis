...
Formación simétrica -> 2 grupos linea -> 2 grupos circulo
"""
...
for k in range(n-1):
    ...
    if estado == 0:
        ...
        [Xr_par,Yr_par,Xir_par] = fx.subLineaForm(V0,w0,theta_act,R0,2,ang_par,N)
        
    elif estado == 2:
        # Formación en linea final 2 grupos
        [Xr_par,Yr_par,Xir_par] = fx.subLineaForm(V0,w0,theta_act,R0,2,ang_f,N)
    
    elif estado ==3:
        # Formacion en circulo 2 grupos
        [Xr_par,Yr_par,Xir_par] = fx.subCircForm(r_act,theta_act,R0,2,0.5,N)
...
if perturbacion == True:
    np.savez('DATA/05_dataPert',robots=N,tiempo=tspan,Xref=Xr,Yref=Yr,Xiref=Xir,xest=x,yest=y,xiest=xi,errx=err_x,erry=err_y,errxi=err_xi,vellin=v,velang=w,veleva=v_eva)
else:
    np.savez('DATA/05_data',robots=N,tiempo=tspan,Xref=Xr,Yref=Yr,Xiref=Xir,xest=x,yest=y,xiest=xi,errx=err_x,erry=err_y,errxi=err_xi,vellin=v,velang=w,veleva=v_eva)