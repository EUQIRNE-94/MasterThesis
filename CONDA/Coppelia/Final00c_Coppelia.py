...
d2 = 0.35 # Radio de seguridad entre robots para evitar colisión
...
for k in range(n-1):
    ...
    if tspan[k] <= 120:
        estado = 0
    elif tspan[k] > 120 and tspan[k] <= 270:
        estado = 1
    elif tspan[k] > 270 and tspan[k] <= 290:
        estado = 2
    elif tspan[k] > 290:
        estado = 3
    
    # Generar el control de partícula e integrar su movimiento
    ## Ingreso y Egreso de particulas
    if estado == 0:
        d2 = 0.4
        u_par = fx.steeringControl(V0,w0,r_ant,theta_ant,R0,kappa,N,M,A1)
        u_par[:,vec[0]] = V0/(rho0/4)
        # u_par[:,vec[1]] = V0/(rho0/2)
...
np.savez('DATA/00c_data',robots=N,tiempo=tspan,Xref=Xr,Yref=Yr,Xiref=Xir,xest=x,yest=y,xiest=xi,errx=err_x,erry=err_y,errxi=err_xi,vellin=v,velang=w,veleva=v_eva)