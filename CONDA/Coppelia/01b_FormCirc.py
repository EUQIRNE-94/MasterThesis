...
data = np.load('DATA/01b_data.npz')
...
fig.suptitle('Velocities',fontsize=26)
axs[0].grid()
axs[1].grid()
axs[0].set_title("Linear velocity")
axs[1].set_title("Angular velocity")
axs[0].set_xlim(right=tf)
axs[1].set_xlim(right=tf)
...
plt.grid()
plt.title('Phase plane',fontsize=26)
...
##### Errores #####
fig, axs = plt.subplots(3,figsize=(15,10))
fig.suptitle('Errors',fontsize=26)
axs[0].grid()
axs[1].grid()
axs[2].grid()
