...
Ks = np.array([[0.8,0.1,0.75,10,0.1,1]])  # [Cx,Cxk,Cxi,Cy,Cyk,K] Ganancias de control del uniciclo
ang_f = 180/(1.5*N) # Angulo de separación entre uniciclos en formacion linea
...
for k in range(n-1):
    ...
    if tspan[k] <= 30:
        estado = 0
    elif tspan[k] > 30 and tspan[k] <= 60:
        estado = 1
    elif tspan[k] > 60 and tspan[k] <= 90:
        estado = 2
    elif tspan[k] > 90:
        estado = 3
    ...
    if estado == 0:
        ...
        # Acercamiento a formacion linea
        ang_par = (360/N) - (tspan[k] - 30)*(ang_f/30)
        [Xr_par,Yr_par,Xir_par] = fx.lineaForm(V0,w0,r_act,theta_act,R0,0,ang_par,N)
        
    elif estado == 2:
        # Formación en linea final
        [Xr_par,Yr_par,Xir_par] = fx.lineaForm(V0,w0,r_act,theta_act,R0,0,ang_f,N)
    
    elif estado ==3:
        # Formacion en V
        [Xr_par,Yr_par,Xir_par] = fx.puntaForm(r_act,theta_act,R0,0,150,0.35,N)
...
if perturbacion == True:
    np.savez('DATA/03_dataPert',robots=N,tiempo=tspan,Xref=Xr,Yref=Yr,Xiref=Xir,xest=x,yest=y,xiest=xi,errx=err_x,erry=err_y,errxi=err_xi,vellin=v,velang=w,veleva=v_eva)
else:
    np.savez('DATA/03_data',robots=N,tiempo=tspan,Xref=Xr,Yref=Yr,Xiref=Xir,xest=x,yest=y,xiest=xi,errx=err_x,erry=err_y,errxi=err_xi,vellin=v,velang=w,veleva=v_eva)